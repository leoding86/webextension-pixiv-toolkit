(function (common, Adapter) {
    ...
    UgoiraEnhancer.prototype = {
        ...
        run: function () {
            ...
            this.downloadResource(context.illustOriginalSrc, {
                onProgress: function (progress) {
                    self.resourceDownloader.button.setText(Math.round(progress * 1000) / 10 + '%');
                }
            }).then(function (zipData) {
                self.zipData = zipData;
                self.downloadResource.button.destroy();

                self.gifGenerator = new GifGenerator(self.zipData, context.illustMimeType, context.illustFrames);
                self.gifGenerator.appendTo(self.wrapper);

                self.webMGenerator = new WebMGenerator(self.zipData, context.illustMimeType, context.illustFrames);
    ...
    function GifGenerator(zipData, mimeType, frames) {
        this.zipData = zipData;
        this.mimeType = mimeType;
        this.frames = frames;
        this.button = new Button(common.lan.msg('generate_gif_btn_text')),
        this.gif,
        this.zip;
            ...
            return new Promise(function (resolve) {
                ...
                if (index < self.frames.length) {
                    self.zip.file(self.frames[index].file).async('base64').then(function (base64) {
                        ...
                        self.gif.addFrame(image, {delay: self.frames[index].delay});
                    ...
                    utils.getImageSize(imageBase64, function (width, height) {
                        ...
                        self.gif.on('finished', function (blob) {
                            ...
                            browser.storage.local.get(null, function (items) {
                                self.button.setAttribute('download', common.formatName(items.ugoiraRenameFormat, context, context.illustId + '_' + context.illustTitle) + '.gif');
                            })
                        });
    ...
    function WebMGenerator(zipData, mimeType, frames) {
        let self = this;
    ...
    WebMGenerator.prototype = {
        ...
        generate: function() {
            ...
            this.zip.loadAsync(zipData).then(function (zip) {
                return zip.file(self.frames[0].file).asunc('base64');
            }).then(function (base64) {
